...
async function sendActivationEmail({ email, name, activationUrl }) {
  ...
  }); } catch (error) { logger.error('Activation email delivery failed', { event: 'EMAIL_FAILED', email: maskEmail(email), emailType: 'activation', ...safeError(error) }); throw error; }
  logger.info('Activation email delivered', { event: 'EMAIL_SENT', email: maskEmail(email), emailType: 'activation' });
...
async function sendPasswordResetEmail({ email, name, resetUrl }) {
  ...
  }); } catch (error) { logger.error('Password reset email delivery failed', { event: 'EMAIL_FAILED', email: maskEmail(email), emailType: 'password-reset', ...safeError(error) }); throw error; }
  logger.info('Password reset email delivered', { event: 'EMAIL_SENT', email: maskEmail(email), emailType: 'password-reset' });
  return { delivered: true };
}
...
async function sendLoginOtpEmail({ email, name, otp }) {
  ...
  try { await transporter.sendMail({
    ...
    html: `<div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;padding:32px;color:#111827"><h1 style="font-size:24px">Confirm your login</h1><p>Hello ${escapeHtml(name)},</p><p>Enter this one-time code to access Pay-Panda:</p><div style="font-size:34px;font-weight:800;letter-spacing:10px;padding:18px;background:#f3f0ff;border-radius:12px;text-align:center">${otp}</div><p style="color:#6b7280;font-size:13px">The code expires in ${config.loginOtpMinutes} minutes and can be attempted five times.</p></div>`,
  }); } catch (error) { logger.error('Login OTP email delivery failed', { event: 'EMAIL_FAILED', email: maskEmail(email), emailType: 'login-otp', ...safeError(error) }); throw error; }
  logger.info('Login OTP email delivered', { event: 'EMAIL_SENT', email: maskEmail(email), emailType: 'login-otp' });
  return { delivered: true };
}
