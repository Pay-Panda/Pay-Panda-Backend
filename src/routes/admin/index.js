...
router.get('/businesses', asyncHandler(async (req, res) => {
  ...
  const [items, total] = await Promise.all([
    prisma.business.findMany({
      ...
      include: { plan: { select: { id: true, name: true } }, _count: { select: { users: true, payments: true } } },
    }),
    prisma.business.count({ where }),
  ]);
  res.json({ success: true, businesses: items, pagination: { page, limit, total, pages: Math.ceil(total / limit) } });
}));
